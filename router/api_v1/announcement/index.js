...
Router.get('/:id', [checkoutIdfromUrl], async (req, res) => {
	try {
		const { id } = req.params;
		const [anno] = await req.mysql._query(
			`SELECT * FROM announcement.announcement WHERE uuid='${id}';`
		);
		if (!anno) return res.status(404).send('Not Found');
		const history = await req.mysql._query(
			`SELECT * FROM announcement.edit_history WHERE uuid='${id}';`
		);
...
Router.delete('/:id', [checkoutIdfromUrl, accept(4)], async (req, res) => {
	try {
		...
		const [ann] = await req.mysql._query(
			`SELECT uuid FROM announcement.announcement WHERE uuid='${id}'`
		);
		if (!ann) return res.status(404).send('Not Found');
		await req.mysql._query(
			`DELETE FROM announcement.announcement WHERE uuid='${id}';`
		);
		res.send('OK');
	} catch (erro) {
		console.log(erro);
...
Router.put(
	...
	async (req, res) => {
		try {
			...
			await req.mysql._query(
				`INSERT INTO announcement.edit_history value (?);`,
				[[id, new Date(), detail, req.auth.uuid, new Date(expire)]]
			);
			res.send('OK');
		} catch (erro) {
			console.log(erro);
